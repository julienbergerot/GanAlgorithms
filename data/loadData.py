...
class imData :
    ...
    # load when inputs and targets are in oimport os
import numpy as np
from PIL import Image
import tensorflow as tf
from tensorflow.keras import layers
...
class imData :
    ...
    def loadClasses(self,key : str,oneHot=True) -> dict :
        ...
        return {"Inputs" : self.iNorm(np.asarray(inputs)), "Labels" : np.asarray(labels), "NumClasses" : len(classes),"Classes" : classes}ne directory intputs are A_*.extension and targets B_*.extensions
    def load(self,key : str) -> dict :
        # get the noum of the files
        self.path = os.path.join(self.path,key)
        assert os.path.isdir(os.path.join(os.path.abspath(os.getcwd()),self.path)) , "There are no directory called {} in this path.".format(self.path)
